...
def get_euler_xyz_tensor(quat):
    ...
    return euler_xyz
class Lite3_legstand(BaseTask):
    def __init__(self, cfg: Lite3Cfg_Leggedstand, sim_params, physics_engine, sim_device, headless):
    ...
    def _compute_torques(self, actions):
        ...
        """
        actions = actions * self.action_scale_per_joint
        # pd controller
    ...
    def _init_buffers(self):
        ...
        # Per-joint action scale.  Lite3 uses a smaller scale for HipX
        # (hip abduction) than for HipY/Knee, mirroring the deploy config in
        # `lite3_push_recovery`.  If `hip_action_scale` is unset the array
        # collapses to a uniform scale and behaves identically to the scalar.
        self.action_scale_per_joint = torch.full(
            (self.num_dof,), float(self.cfg.control.action_scale),
            dtype=torch.float, device=self.device, requires_grad=False,
        )
        hip_scale = getattr(self.cfg.control, "hip_action_scale", None)
        if hip_scale is not None:
            for i, dof_name in enumerate(self.dof_names):
                if "HipX" in dof_name:
                    self.action_scale_per_joint[i] = float(hip_scale)
    ...
    def _reward_handstand_feet_height_exp(self):
        # Lite3 rear-foot world-z target during handstand (vs Go2's 0.67 — Lite3
        # legs are shorter so the swung-up rear foot rests lower).
        # feet_indices_tensor = torch.tensor(feet_indices, dtype=torch.long, device=self.rigid_body_pos.device)
        feet_height_error = torch.abs(self.feet_pos[:,:,2]- 0.56).sum(dim=1)
