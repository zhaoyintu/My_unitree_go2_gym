...
class Lite3IsaacGymMjlabGatedTrackingFrameStackStaticTest(unittest.TestCase):
    ...
    def test_frame_stack_env_stacks_actor_and_critic_observations(self) -> None:
        ...
        self.assertIn("self.single_noise_scale_vec", init_source)
        self.assertIn("self.noise_scale_vec[", init_source)
        self.assertIn(":self.cfg.env.num_single_obs", init_source)
        self.assertIn("].clone()", init_source)
        self.assertIn("self.noise_scale_vec = self.single_noise_scale_vec", init_source)
