"""Unitree Go2 legged stand environment configuration.

A specialized task where the robot stands on its rear legs (RL, RR) with
front legs raised. Similar to handstand but with upright orientation.
"""
...
def unitree_go2_leggedstand_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    """Create Unitree Go2 legged stand configuration."""
    foot_names = ("FR", "FL", "RR", "RL")
    rear_foot_names = ("RR", "RL")
    geom_names = tuple(f"{n}_foot_collision" for n in foot_names)
    rear_geom_names = tuple(f"{n}_foot_collision" for n in rear_foot_names)
    ...
    rewards = {
        "upright": RewardTermCfg(
            func=go2_mdp.leggedstand_orientation, weight=1.0,
            params={"asset_cfg": SceneEntityCfg("robot", body_names=("trunk",))},
        ),
        "front_feet_air": RewardTermCfg(
            func=go2_mdp.leggedstand_feet_on_air, weight=0.5,
            params={
                "sensor_name": "feet_ground_contact",
                "foot_indices": (0, 1),  # FR, FL
            },
        ),
